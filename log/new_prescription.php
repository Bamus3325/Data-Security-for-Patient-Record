...
if(isset($_POST['submit'])) {
  ...
  $remain = $_POST['remain'];
    ...
    // $quer = mysqli_query($con, "UPDATE medicine_details SET total = '$remain' WHERE id = '$med_id'");

    
$query = $con->prepare("UPDATE medicine_details SET total = :remain WHERE id = :med_id");

// Bind the parameters to the placeholders
$query->bindParam(':remain', $remain, PDO::PARAM_INT);
...
<head>
    ...
    <title>New Prescription - Olalomi Patient Management System</title>
                        ...
                        <form method="post">
                            ...
                            <div class="row">
                                <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
                                    ...
                                    <!-- <select id="medicine" class="form-control form-control-sm rounded-0" onchange="calculateTotal()"> -->
                                ...
                                <div class="col-lg-2 col-md-2 col-sm-6 col-xs-12">
                                    ...
                                    <input id="tot" class="form-control form-control-sm rounded-0"
                                        oninput="calculateTotal()" readonly />
                                </div>
                                <!-- <label>Med</label> -->
                                    <input type="hidden" id="med_id" name="med_id" class="form-control form-control-sm rounded-0"
                                        readonly />
                                ...
                                <div class="col-lg-2 col-md-2 col-sm-6 col-xs-12">
                                    ...
                                    <input id="quantity" class="form-control form-control-sm rounded-0"
                                        oninput="calculateTotal()"  /><br><br>
                                </div>
                                <div class="col-lg-2 col-md-2 col-sm-6 col-xs-12">
                                    <!-- <label>Remaining</label> -->
                                    <input type="hidden" id="remain" name="remain" class="form-control form-control-sm rounded-0" readonly /><br>
                                </div>


                                <button type="button" id="submitButton" class="form-control" onclick="checkTotal()">Check</button>

                                <div id="errorMessage" style="color: red; font-family: arial black; display: none;"></div>
                                <div id="nameInputs" style="display: none;" class="row">
...
$(document).ready(function() {
    $('#packing').change(function() {
        var stdid = $('#packing').val();
        //  alert (stdid);


        $.ajax({
            type: "POST",
            url: "ajax/get_total.php",
            data: {
                id: stdid
            },
...
function calculateTotal() {
    ...
    const numberInput = document.getElementById("tot").value;
    // Get the value of the selected option
    const selectOption = document.getElementById("quantity").value;
    //alert(numberInput);
    // Ensure numberInput is a valid number, otherwise default to 0
    const number = parseFloat(numberInput) || 0;
    // Convert selectOption to a number
    const option = parseInt(selectOption);

    // Calculate total
    const total = number - option;

    // Display the total in the "total" input
...
function checkTotal() {
    ...
    if (quantity > tot) {
        // Show an error message if total is greater than 20
        document.getElementById("errorMessage").innerText = "Error: The amount of quantity is not availabale!";
        document.getElementById("errorMessage").style.display = "block";
        // Hide the name inputs if there's an error
        document.getElementById("nameInputs").style.display = "none";
        // Ensure submit button is visible again if total is invalid
        document.getElementById("submitButton").style.display = "inline-block";
    } else {
        // Hide the error message if total is not greater than 20
        document.getElementById("errorMessage").style.display = "none";
        // Show the name inputs and hide the submit button
